...
class IssueViewSet(ModelViewSet):
    ...
    def perform_update(self, serializer):
        ...
        def send_email(instance):
            ...
            email_body = '<b>Issue:</b> '+instance.issue_title+' <b>Updated Status: </b>'+instance.issue_status+' <b>Project:</b> '+instance.issue_project.project_name
            email_sender = 'spuggy.sparsh@gmail.com'
            recipents = []
            ...
            send_mail(email_subject, email_body, email_sender, recipents, fail_silently=True);    
    ...
    def perform_create(self, serializer):
        ...
        def send_email(instance):
            ...
            email_body = '<b>Issue:</b> '+instance.issue_title+' <b>Project:</b> '+instance.issue_project.project_name
            email_sender = 'spuggy.sparsh@gmail.com'
            recipents = []
            recipents.append(instance.issue_project.created_by.profile.email)
            for team_member in instance.issue_project.team_members.all():
...
class CommentViewSet(ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        def send_email(instance):
            ...
            email_body = '<b>Comment:</b> '+instance.comment_text+' <b>Issue:</b> '+instance.comment_issue.issue_title+' <b>Project:</b> '+instance.comment_project.project_name
            email_sender = 'spuggy.sparsh@gmail.com'
            team_members_recipents = []
            team_members_recipents.append(instance.comment_issue.created_by.profile.email)
            for team_member in instance.comment_project.team_members.all():
                team_members_recipents.append(team_member.profile.email)
            send_mail(email_subject, email_body, email_sender, team_members_recipents, fail_silently=True)
        email_thread = threading.Thread(target = send_email, args=(instance,))
        email_thread.start()
...
def oauth_handler(request):
    ...
    resp = requests.post('https://internet.channeli.in/open_auth/token/', data=data_for_post_request1)
    ...
    user_data = requests.get('https://internet.channeli.in/open_auth/get_user_data/', headers=header)
    ...
    custom_username = str(current_year)+'_'+name[0]+last_char_enrollment_number[4]+last_char_enrollment_number[5]+last_char_enrollment_number[6]+last_char_enrollment_number[7]
    ...
    try:
        ...
        obj.save()
        obj2 = Profile(user=obj, current_year=current_year, name=name, branch=branch, email=email )
        obj2.save()
